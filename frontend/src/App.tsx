...
const App = () => {
  ...
  const [gIndexTotalSupply, setgIndexTotalSupply] = useState(0)
  const [gIndexContract, setgIndexContract] = useState<ExpandedIERC20>()
  ...
  useEffect(() => {
    refreshBalances()
  }, [gIndexContract, daiContract]) //eslint-disable-line
  ...
  const refreshBalances = async () => {
    let gIndexBalance = balances.UBE
    let daiBalance = balances.DAI

    if (gIndexContract) {
      try {
        const totalSupply = await gIndexContract.totalSupply()
        console.log('totalSupply:', totalSupply)
        setgIndexTotalSupply(bigNumberToFloat(totalSupply))

        if (userAddress) {
          const bal = await gIndexContract.balanceOf(userAddress)
          console.log('UBE balance:', bal)
          gIndexBalance = bigNumberToFloat(bal)
        }
      } catch (err) {
        console.error(err)
      }
    ...
    setBalances({ ...balances, UBE: gIndexBalance, DAI: daiBalance })
  }
              ...
              <Box p={2}>
                ...
                <Box mt={2} textAlign="center">
                  ...
                  <Typography variant="h2">
                    {formatBalance(gIndexTotalSupply)}
                  </Typography>
                </Box>
